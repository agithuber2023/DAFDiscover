...
class PartitionsManager(object):
    ...
    def check_fd(self, X, XY, r, error_list):
        ...
        return bool(calculate_e(X, XY.attr_set, range(r), self) <= alpha(error_list, XY.attr_set, r))
        ...
        for X in L:
            for y in self.Cplus[X.attr_set].intersection(X.attr_set):
                ...
                if self.pmgr.check_fd(LHS, X, r, error_list):
                    # self.rules.append(((LHS, y), 1-calculate_e(LHS, X.attr_set, range(r), self.pmgr)))
...
if __name__ == "__main__":
    #T , r= read_db('data/raisin/raisin-enhanced-10-10000-dirty.csv')
    T, r=read_db('data2/chess-dirty-20.csv')
    ...
    # error_list=[0,0,0,0,0.1,0,0,0.04,0.02] #abalone
    # error_list=[0,0,0.03,0,0,0.03,0.01] #chess*10(P:1, R:1)
    error_list=[0,0,0.06,0,0,0.06,0.02] #chess*20
    # error_list=[0,0,0.09,0,0,0.09,0.03] #chess*30
    # error_list=[0, 0.06, 0, 0.04, 0, 0.1, 0, 0.06, 0, 0.08, 0.02] #breast-cancer-wisconsin
    # error_list=[0, 0.2, 0, 0, 0, 0.05, 0.2, 0.1] #raisin(P:0.878, R:1)
    # error_list=[0, 0, 0.05, 0, 0.1, 0.02, 0, 0, 0, 0, 0, 0, 0, 0, 0.2, 0.1] #day
    # error_list=[0, 0, 0.05, 0, 0.02, 0, 0, 0.2, 0.1] #day-new
    # error_list=[0, 0.02, 0, 0, 0, 0.02, 0.0667, 0.0333] #raisin-10
    # error_list=[0, 0.04, 0, 0, 0, 0.04, 0.1333, 0.0667] #raisin-20
    # error_list=[0, 0.06, 0, 0, 0, 0.06, 0.2, 0.1] #raisin*30
    # error_list=[0, 0, 0.03, 0, 0.02, 0, 0, 0.18, 0.08] #day-new*10
    # error_list=[0, 0, 0.06, 0, 0.04, 0, 0, 0.36, 0.16] #day-new*20
    # error_list=[0, 0, 0.09, 0, 0.06, 0, 0, 0.54, 0.24] #day-new*30
    ntane = DAFDiscover(T)
