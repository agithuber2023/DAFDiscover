...
class PartitionsManager(object):
    ...
    def check_fd(self, X, y, r, support):
        '''
        ...
        return bool(calculate_e(X, (y,), range(r), self) >=support)
...
def calculate_e(X, Y, R, checker):
    ...
    return (mutual_info_score(x, y)+0.0069)/(mutual_info_score(y, y)+0.0069)
...
class MIDD(object):
    ...
    def compute_dependencies(self, L, r, support):
        '''
        ...
        for X in L:
            for y in self.Cplus[X].intersection(X):
                a = X.index(y)
                LHS = X[:a]+X[a+1:]
                if self.pmgr.check_fd(LHS, y, r, support):
    ...
    def run(self, r, support):
        ...
        l = 1 
        while bool(L[l]):
            self.compute_dependencies(L[l], r, support)
            self.prune(L[l])
...
if __name__ == "__main__":
    T , r= read_db('abalone-dirty-x/abalone-dirty-32000.csv')
    support=0.995
    midd = MIDD(T)
    ...
    print ('\t=> {} Rules Found'.format(len(midd.rules)))
    # for item in midd.rules:
    #     print(item)
    # TT , rr=read_db('data/breast-cancer-wisconsin/breast-cancer-wisconsin-enhanced-10.csv')
    # nntane = tane.TANE(TT)
    # nntane.run()
    # print ('\t=> {} Rules Found'.format(len(nntane.rules)))
    # precision=1-(len(set(midd.rules)-set(nntane.rules))/len(set(midd.rules)))
    # print("P:", precision)
    # print('hahahaha')
    # recall=1-(len(set(nntane.rules)-set(midd.rules))/len(set(nntane.rules)))
    # print("R:", recall)
